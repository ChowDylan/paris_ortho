...
print json.dumps(order_template, indent=4)

summon_ferret()


# my_order = copy.deep_copy(order_template)
# my_order["patient_name"]["first"] = "MARK"
# my_order["patient_name"]["last"] = "BONNELL"
# my_order["po_num"] = 108132
# my_order["shell"]["bilateral"]["shell_material"] = "PRx-2"
#
...
def process_po_number(curr_order, row):
    # print '===================================='
    # Creation of the dictionary [why here, why not higher]
    curr_order = copy.deepcopy(order_template)
    ...
    curr_order['patient_name']['raw_name'] = row[1].value.strip()
    ...
    # print nameList
    if curr_order['patient_name']['raw_name'] == '':
        curr_order['derived']['issue_list'].append('name missing')

    curr_order['patient_name']['number'] = ''
    namespace = ' '
    if len(nameList) == 1:
        namespace = ''

    if len(nameList) >= 1:
        curr_order['patient_name']['first'] = " ".join(nameList[0:-1])
        curr_order['patient_name']['last'] = nameList[-1]

    if len(nameNumber) == 1:
        curr_order['patient_name']['number'] = nameNumber[0]

    if len(nameNumber) > 1:
        curr_order['patient_name']['number'] = nameNumber[0]

    curr_order['patient_name']['first'] = (curr_order['patient_name']['first'] +
                                           namespace +
                                           curr_order['patient_name']['number'])
    return curr_order
...
for xfile in xlsfiles:
    ...
    for order in range(xl_sheet.nrows):
        ...
        if row[0].value == 'PATIENT NAME / CODE NO.':
            process_po_number(curr_order, row)

            for prev_order in orders:
                # Check previous orders for exact raw_name
                # might need to be refined to compare first name and last name separately
                if curr_order['patient_name']['raw_name'] == prev_order['patient_name']['raw_name']:
                    curr_order['derived']['issue_list'].append('Buddy')
                    curr_order['derived']['buddy_order'] = 'yes'
                    curr_order['derived']['suborder_target'] = prev_order['patient_name']['raw_name']
        ...
        if row[0].value == 'QUANTITY / SUBSEQUENT PAIR':
            ...
            curr_order['quantity'] = curr_order['quantity'].replace(".0", "")
...
oe_gate = 'y1es'  # Gates OE part  (needs to be changed to something better for all orders)
...
def patientCardHandler(order):
    ...
    if order['sameday_suborder'] == 'yes':
        # Scan list for
        prev_order = [for x in orders if x['patient_name']['raw_name'] == order['derived']['suborder_target']][0]
        pyautogui.typewrite(prev_order['derived']['pt_num'])
...
def deviceSelection(order):
    ...
    if order['sameday_suborder'] == 'yes':
        ...
        # This assumes changed order, which is correct why not say 2 pairs
        # if order['device_mod'] == 'yes':
        #     pyautogui.hotkey('alt', 'm')
        #     pyautogui.typewrite(['down', 'down', 'down', 'down', 'down', 'down'])
        #     q_plus = order['quantity_num'] - 1
        ...
        if order['sub_order'] == 'new order':
            ...
            # if order['device_mod'] == 'yes':
            #     pyautogui.hotkey('alt', 'm')
            #     pyautogui.typewrite(['down', 'down', 'down', 'down', 'down', 'down', 'right'])
            #     pyautogui.typewrite(order['footedness'])
            pyautogui.typewrite(['return', 'return'], interval=SHORT_DELAY)
            while pyautogui.locateOnScreen(remove_button, confidence=0.97, region=(1070, 378, 109, 60)) is None:
                if count > 12:
                    if pyautogui.locateOnScreen(deadlock, confidence=0.97,
                ...
                if ORDER_START <= k <= ORDER_LIMIT:
                    ...
                    if order['sameday_suborder'] == 'no':
                        ...
                        if continue_code == '1':
                            ...
                            image = pyautogui.screenshot('subfetch_{}.png'.format(order['po_num']))
                            continue
                    ...
                    if continue_code == '1':
                        ...
                        image2 = pyautogui.screenshot('ocreation_{}.png'.format(order['po_num']))
                        continue
                    ...
                    if continue_code == '1':
                        ...
                        image3 = pyautogui.screenshot('ptcard{}.png'.format(order['po_num']))
                        continue
                    ...
                    if continue_code == '1':
                        ...
                        image4 = pyautogui.screenshot('device_{}.png'.format(order['po_num']))
                        continue
                    ...
                    if sticker_gate == 'yes':
                        if order['data_base'] is not 'LFE':
                            ...
                            while pyautogui.locateOnScreen(after_print, confidence=0.97, region=(0, 920, 1920, 160)) is None:
                                ...
                                time.sleep(0.8)
                                if marker > 10:
                                    if pyautogui.locateOnScreen(status_history, confidence=0.97, region=(644, 387, 572, 320)):
                                        image5 = pyautogui.screenshot('label_{}.png'.format(order['po_num']))
                                        pyautogui.typewrite(['return'])
                                        pyautogui.hotkey('alt', 'e')
                                    if pyautogui.locateOnScreen(deadlock, confidence=0.97,
                                                                region=(644, 387, 572, 320)):
                                        image5 = pyautogui.screenshot('deadlock_{}.png'.format(order['po_num']))
                                        pyautogui.typewrite(['return'])
                                        pyautogui.hotkey('alt', 'e')
                                marker += 1
                            else:
    ...
    for order in orders:
        ...
        if len(order['issue_list']) >= 1:
            if order['nameLast'] != '':
                #print order['wo_num']
                line = " ".join(map(str,[
                    '[WO#', order['wo_num'] + ']', order['FIRSTNAMECOMPLETE'], order['nameLast'],
                    '[PO#', order['po_num'] + ']', order['issue_list'], "\n\n"
                ]))
                print '[WO#', order['wo_num'] + ']', order['FIRSTNAMECOMPLETE'], order['nameLast'], \
                    '[PO#', order['po_num'] + ']', order['issue_list']
