...
order_search = Image.open('order_search.bmp')
pt_search = Image.open('patient_search.bmp')
post_dress = Image.open('post_dress.bmp')
posted_orders = Image.open('posted_orders.bmp')
pt_card = Image.open('pt_card.bmp')
remove_button = Image.open('remove_button.bmp')
special_instructions = Image.open('special_instructions.bmp')
start_screen = Image.open('start_screen.bmp')
...
def fetchSubsequentOrder(order):
    #TODO Add device grabber
    pyautogui.typewrite(['down', 'return']) # selects order search
    # todo image find posted sales search pyauto click
    location = pyautogui.locateCenterOnScreen('posted_orders')
    # todo py auto click in po num search collumn, posted sales search could just be organized so that po is the very left
    # todo and the other points of interest are next to it too
    center = pyautogui.center(location)
    below_center = center + (0,100)
    pyautogui.click(below_center)
    pyautogui.typewrite(['left', 'left', 'left', 'left', 'left'])
    pyautogui.typewrite(order['prev_po'])
    pyautogui.typewrite(['return', 'right'])
    pyautogui.hotkey(['ctrl', 'c'])
    order['prev_wo'] = (Tk().clipboard_get())
    pyautogui.typewrite(['right'])
    pyautogui.hotkey(['ctrl', 'c'])
    order['prev_pt'] = (Tk().clipboard_get())
    pyautogui.typewrite(['right'])
    pyautogui.hotkey(['ctrl', 'c'])
    order['prev_device'] = (Tk().clipboard_get()) #device grabber
...
def orderCreation(order):
    ...
    time.sleep(3)
    # Enter clinician for Firefly
    # TODO: Search for Martin's clinician number
...
def footImpressionAndPONumberEntry(order, orders, quantity=False):
...
def deviceSelection(order):
    DEVICE_SELECTION_DELAY = 12 # TODO: To replace with image recognition
...
for i in range(len(orders)):
    ...
    if order['po_num'] != '': # doesn't make orders for blank orders
        if order['sub_order'] == 'new order88':
            if k <= ORDER_LIMIT:
...
def printOrdersOfInterest(orders):
    ...
    for order in orders:
        if len(order['issue_list']) >= 1:
            ...
            #print i['wo_num']
            print order['FIRSTNAMECOMPLETE'], order['nameLast'], ' :', order['issue_list']
