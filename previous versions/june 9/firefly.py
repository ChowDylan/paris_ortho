...
LAG = 23  # Delay for in and out of pt search default: 40, 23
oe_gate = 'new order88'  # Gates OE part  (needs to be changed to something better for all orders)
...
def printOrderIntoCard(order, k):
    if order['po_num'] != '':
         if order['sub_order'] == 'new order' or 'duplicate' or 'changed':
            ...
            print 'Estimated minutes to complete Batch ',(k * 143)/60
...
def fileRenamer(order):
    ...
    if order['foot2scan'] == 'BOTH':
        ...
        print old_path
        new_path = raw_location + order['wo_num'] + order['device_code'] + foot_letter + '.RAW'
        print new_path
