...
oe_gate = 'new orde8r'  # Gates OE part  (needs to be changed to something better for all orders)
...
def fileRenamer(order):
    ...
    def checkAndRenameFoot(foot):
        ...
        if clean_expected_file in cleanfile_to_rawfile:
            ...
            order['new_path' + foot] = raw_location + order['wo_num'] + foot + '.RAW'
            ...
            if k <= ORDER_LIMIT:
                ...
                if sticker_gate == 'yes':
                    if order['data_base'] is not 'LFE':
                        ...
                        while pyautogui.locateOnScreen(after_print, confidence=0.97, region=(1920, 1080, 1920, 1080)) is None:
