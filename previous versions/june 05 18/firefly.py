...
posted_button = Image.open('posted_button.bmp')
pt_popup = Image.open('pt_popup.bmp')
pt_search = Image.open('pt_search.bmp')
...
def fetchSubsequentOrder(order): #bannana grabber tm
    ...
    pyautogui.click(pyautogui.locateCenterOnScreen(posted_button, region=(1264, 164, 150, 56)))
    ...
    pyautogui.typewrite(['return'])
    time.sleep(1)
    if pyautogui.locateCenterOnScreen(sub_search_confirm) == True:
...
def patientCardHandler(order):
    if order['sameday_suborder'] == 'yes':
        ...
        if order['sub_order'] == 'new order':
            ...
            found = 'no'
            while found == 'no':
                print 'Looking for Pt Search Screen #', x
                if pyautogui.locateOnScreen(pt_search) != None:
                    found = 'yes'
                time.sleep(x)
                x = x + 1
                if x == 8:
            ...
            while found == 'no':
                ...
                if pyautogui.locateOnScreen(pt_popup) != None:
                    found = 'yes'
                    time.sleep(x)
                ...
                if pyautogui.locateOnScreen(similar_ptname) != None:
                    order['issue_list'].append('Similar Pt Name Issue')
                    pyautogui.hotkey('shift', 'f5')
                    pyautogui.press('f3', 'tab')
                    pyautogui.typewrite(i['FIRSTNAMECOMPLETE'])
                    pyautogui.typewrite(['tab'])
                    pyautogui.typewrite(i['nameLast'])
                    pyautogui.press(['tab', 'm', 'tab'])
                    pyautogui.typewrite(order["dob"])
                    pyautogui.typewrite(['tab'])
                    pyautogui.typewrite(order['weight'])
                    pyautogui.typewrite(['tab'])
